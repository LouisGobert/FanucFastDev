...
namespace Compilator.Interpreter
{
    public class Interpreter
    {
        ...
        private static bool keepBlankLine;
        ...
        public static string start(string newSavePath, bool setKeepBlankLine)
        {
            // Ouverture de FanucFastDev.cs
            fileLine = FileGestion.FileGestion.getFileLine(newSavePath);
            savePath = newSavePath;
            keepBlankLine = setKeepBlankLine;
        ...
        private static void lsProgramInterprete(string programName)
        {
            ...
            addLine(s);
            addLine($"Generation.setup(\"{programName}\", @\"{savePath}\");\n");

            //Console.WriteLine(s);
            //Console.WriteLine($"\tGeneration.setup(\"{programName}\", \"{savePath}\");");
            //sb.Append(s + "\n");
            //sb.Append($"Generation.setup(\"{programName}\", @\"{savePath}\");\n");
            ...
            for (; iFL < indexProgramEnd; iFL++)
            {
                ...
                // Si la ligne est vide et que l'on garde les lignes vides
                if (keepBlankLine && s.Trim().Length == 0)
                {
                    blankLineCount = 0;
                    do
                    {
                    ...
                    //Console.WriteLine($"\tGeneration.appendXBlankLine({blankLineCount});");
                    //sb.Append($"Generation.appendXBlankLine({blankLineCount});");
